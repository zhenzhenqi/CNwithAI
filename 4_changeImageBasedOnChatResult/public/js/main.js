...
messageForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        // --- 5. Display the AI's response using "mumblejumble" attribute ---
        const botMessage = { role: 'assistant', content: data.mumblejumble };
        displayMessage(botMessage);
        statusEl.textContent = ''; // Clear status

        // Bonus: Check if the conversation should end
        if (data.isConvoEnd) {
            statusEl.textContent = 'Conversation ended. Refresh to start a new one.';
